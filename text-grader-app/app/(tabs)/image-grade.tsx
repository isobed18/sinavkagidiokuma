...
import { StyleSheet, Text, View, Button, Alert, ScrollView, TextInput, Image, KeyboardAvoidingView, Platform, TouchableOpacity } from 'react-native';
...
interface FullPageStructuredContent {
  question: string;
  answer: string;
}

interface FullPageErrorResponse {
  error: string;
  raw_response: string;
}
...
interface FullPageGradingResponse {
  ...
  structured_content: FullPageStructuredContent[] | FullPageErrorResponse;
  processing_times_ms: {
    llama_vision: number;
          ...
          <View style={styles.resultContainer}>
            ...
            {gradingType === 'single' ? (
              ...
              <>
                ...
                {Array.isArray(result.structured_content) ? (
                  ...
                  <View style={styles.structuredItem}>
                    <Text style={{ fontWeight: 'bold', color: 'red' }}>Hata:</Text>
                    <Text>{result.structured_content?.error || "LLM'den beklenmedik bir yanıt formatı."}</Text>
                    <Text style={{ fontWeight: 'bold', marginTop: 5 }}>Modelin Ham Yanıtı:</Text>
                    <Text>{result.structured_content?.raw_response || "Yok"}</Text>
                  </View>
                )}
              </>
            )}
